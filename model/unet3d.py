...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[2, 4], bottleneck_channel=8) -> None:
    ...
    def forward(self, input):
        ...
        #print("Block 1: ", np.shape(out), np.shape(residual_level1))
        out, residual_level2 = self.a_block2(out)
        #print("Block 2: ", np.shape(out), np.shape(residual_level1))
        #out, residual_level3 = self.a_block3(out)
        out, _ = self.bottleNeck(out)
        print("===Bottle Neck===")
        print("Block BottleNeck: ", np.shape(out))

        #Synthesis path forward feed
        #out = self.s_block3(out, residual_level3)
        #print("===Expansive Path===")
        out = self.s_block2(out, residual_level2)
        #print("Block 2: ", np.shape(out))
